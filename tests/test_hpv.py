...
def _planted_cohort() -> pd.DataFrame:
    """HPV+ clearly outlives HPV- (few late events vs many early events)."""
    rng = np.random.default_rng(42)
    pos = pd.DataFrame({
        "submitter_id": [f"P{i:02d}" for i in range(30)],
        "hpv_status": "positive",
        "os_days": rng.normal(2000, 150, 30).round(1),
        "os_event": ([0] * 27) + ([1] * 3),  # 10% events
    })
    neg = pd.DataFrame({
        "submitter_id": [f"N{i:02d}" for i in range(30)],
        "hpv_status": "negative",
        "os_days": rng.normal(700, 150, 30).round(1),
        "os_event": ([1] * 24) + ([0] * 6),  # 80% events
    })
...
class TestHpvSurvivalSummary:
    def test_planted_signal_is_protective(self):
        ...
        assert s["n_hpv_positive"] == 30
        assert s["n_hpv_negative"] == 30
